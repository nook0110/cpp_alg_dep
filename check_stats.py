...
def merge_stats(stats_list):
    ...
    for stats in stats_list:
        for key in ['total', 'with_dependency', 'trivial_rejected', 'nontrivial_found',
                    'df_divisible_only', 'dg_divisible_only', 'both_divisible', 'no_dependency']:
            merged[key] += stats[key]
        
        for detail in stats.get('detailed', []):
            key = (detail['has_dependency'], detail['is_nontrivial'],
                   detail['df_divisible'], detail['dg_divisible'], detail['both_divisible'])
...
def query_polynomials(db_files, has_dep=None, is_nontrivial=None, df_div=None, dg_div=None, both_div=None, limit=None):
    results = []
    ...
    for db_file in db_files:
        ...
        try:
            ...
            query = "SELECT f_poly, g_poly, q_poly, is_trivial, df_divisible, dg_divisible, both_divisible FROM results WHERE 1=1"
            params = []
...
Examples:
  ...
  python check_stats.py data/*.db --show --no-dep --limit 5
    ...
    if args.show:
        ...
        results = query_polynomials(db_files, has_dep, is_nontrivial, df_div, dg_div, both_div, args.limit)
